...
module.exports = function (req, res, next) {
    ...
    if (naughtiness >= 0.5) {
        // Let this request stew a while (up to about 20 seconds) before we
        // respond.
        res.setHeader('RawGit-Message', "Please enhance your calm or this request will be blacklisted soon.");
...
function blacklist(req, res) {
    ...
    res.type('txt').send('This request has been blacklisted. Stop abusing ' + config.devDomain + ' or worse things will happen soon.');
}
...
function evil(req, res) {
    ...
    default:
        res.type('txt').send('This request has been blacklisted. Stop abusing ' + config.devDomain + '.');
    }
}
