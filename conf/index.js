...
module.exports = {
  ...
  // referrer. This is used to give referrers more leeway than individual files,
  // since they're often legitimately requesting multiple files per page.
  ...
  // if you use monitoring tools that ping a given URL every few minutes and you
  // don't want them affecting stats rankings.
  statsIgnorePaths: {
    '/rgrove/rawgit/master/web.js': true
  },

  // If RawGit is fronted by Nginx, Apache, or something else that generates
  // logs in Common/Combined Log Format, set the path to that file here to have
  // RawGit tail the log and use it for accurate request statistics.
  //
  // If this is not set or if the file doesn't exist or isn't readable, RawGit
  // will track requests internally (but this may result in inaccurate stats if
  // you're fronting RawGit with a caching proxy).
