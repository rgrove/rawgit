...
function parseLine(line) {
    ...
    var time = Date.parse(matches[2].replace(REGEX_DATE, '$1 $2')) || 0;
...
function tail(logPath, respawn) {
    ...
    var tailProcess = spawn('tail', ['-F', '-n', scrollBack, logPath]);
    var tailStream  = tailProcess.stdout;
    ...
    tailStream.on('readable', function () {
        ...
        var lines = (overflow + chunk).split("\n");
