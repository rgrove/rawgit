...
@type RegExp
**/
var REGEX_DIRECTIVE = /^!rawgit-redirect\s+((?:https?:)?[^:\s]+)(?:\s+(301|302))?\s*;\s*$/i;
...
RedirectTransform.prototype._flush = function (callback) {
    if (this._bufferedChunks.length) {
        this._bufferedChunks.forEach(function (bufferedChunk) {
...
RedirectTransform.prototype._transform = function (chunk, encoding, callback) {
    if (this._finished) {
        if (this._skipRemainingChunks) {
            // We've already found and processed a redirect directive, so don't
            // pass any chunks through.
            return void callback();
        }

        // We've finished looking for a redirect directive and didn't find one,
        // so just pass all chunks through as quickly as possible.
    ...
    var chunkString = chunk.toString('utf8', 0, 1024 - this._directiveByteLength);
    var directive   = this._directive += chunkString;
    ...
    var matches = directive.match(REGEX_DIRECTIVE);
